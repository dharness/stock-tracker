// Service to fetch real stock data from Massive API or static JSON file
// Documentation: https://massive.com/docs/rest/quickstart
// In production, data is fetched daily by GitHub Actions and saved to public/data/stocks.json
...
/**
 * Attempts to load stock data from static JSON file (generated by GitHub Actions)
 * @param symbol Stock ticker symbol
 * @param year Year to fetch data for
 * @returns PriceData array if found, null otherwise
 */
const loadFromStaticData = async (
  symbol: string,
  year: number
  ...
  try {
    ...
    if (!response.ok) {
      return null; // File doesn't exist or can't be loaded
    }

    const data: StaticStockData = await response.json();
    const key = `${symbol}_${year}`;
    return data.stocks[key] || null;
  } catch (error) {
...
export const fetchStockData = async (
  ...
  // First, try to load from static JSON file (generated by GitHub Actions)
  const staticData = await loadFromStaticData(symbol, year);
  if (staticData && staticData.length > 0) {
    console.log(`Loaded ${symbol} (${year}) from static data`);
    return staticData;
  }

  // Fall back to API if static data is not available
  console.log(`Fetching ${symbol} (${year}) from API...`);
